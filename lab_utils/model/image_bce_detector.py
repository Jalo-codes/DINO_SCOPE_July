...
class AttentionPool(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attention: bool = False,
        ...
        # x: (B, N, D).  keep_mask: (B, N) or (N,) bool, True = visible patch.
        # None ⇒ every patch visible (current/training behavior, untouched).
        h = torch.tanh(self.V(x)) * torch.sigmoid(self.U(x))  # (B, N, d_hidden)
        s = self.w(h)                                          # (B, N, 1) pre-softmax score
        if keep_mask is not None:
            km = keep_mask
            if km.dim() == 1:
                km = km.unsqueeze(0).expand(x.shape[0], -1)    # (N,) → (B, N)
            km = km.to(torch.bool)
            if not bool(km.any(dim=1).all()):
                raise DataError('AttentionPool: keep_mask hides all patches in a row')
            # fp32 for a stable masked softmax; hidden patches → -inf → 0 weight,
            # surviving patches renormalize to sum 1.
            s = s.float().masked_fill(~km.unsqueeze(-1), float('-inf'))
        a = s.softmax(dim=1)                                   # (B, N, 1)
