"""lab_utils.data.loaders — DataLoader factories for Dataset batches."""
...
class DistributedRandomSubsetSampler(Sampler):
    ...
    ``train_samples_per_epoch`` is a global cap across all ranks. Each rank
    receives a deterministic shard; ``set_epoch()`` changes the shuffle order
    the same way PyTorch's DistributedSampler does.
    """
...
class DistributedWeightedSampler(Sampler):
    ...
    ``num_samples`` is a global target across all ranks. Sampling is
    deterministic per epoch so every rank sees a disjoint shard of the same
...
def build_train_loader(dataset, cfg: LoaderConfig) -> DataLoader:
    """Build a randomized Dataset loader.

    Args:
        dataset: Dataset or compatible dataset returning {img, mask, meta} dicts.
...
def build_eval_loader(dataset, cfg: LoaderConfig) -> DataLoader:
    """Build a deterministic sequential Dataset loader."""
    sampler = (
        DistributedSampler(
            dataset,
            num_replicas=int(cfg.world_size),
...
__all__ = [
    ...
    'DistributedRandomSubsetSampler',
]
