...
TORCH-FREE (GAMEPLAN C3). Ported from legacy/lab_utils/data/augment/degradation.py.

Key change from legacy: DegradationExample stores ``mask: Optional[Image.Image]``
instead of ``labels: torch.Tensor``.  The Dataset (or any caller) is responsible
for converting the mask to patch labels via ``mask_to_patch_labels`` at
__getitem__ time.  This keeps the entire augmentation pipeline torch-free.

  legacy:  DegradationExample.labels  = torch.Tensor  (torch-bound at import)
  rebuild: DegradationExample.mask    = Optional[Image.Image]  (torch-free)
"""
...
from lab_utils.data.augment.blob import EllipseBlobParams, generate_blob_mask_pil
...
class DegradationExample:
    ...
    Attributes:
        image:      Degraded PIL RGB image (res.image_size × res.image_size).
        mask:       PIL 'L' mask of the TARGET (anomalous) region, or None when
                    the entire image is uniformly degraded (global_negative).
                    The caller converts this to patch labels via
                    mask_to_patch_labels(example.mask, res, threshold) —
                    if mask is None, all labels are 0.
        supervised: True when the example has a meaningful contrastive signal.
        is_single:  True for global-only variants (no local region).
        meta:       Diagnostic metadata for logging / analysis.
        applied:    AppliedOp records for every corruption that ran.
    """
    image:      Image.Image
    mask:       Optional[Image.Image]   # None ⇒ all-background (global_negative)
    supervised: bool
    is_single:  bool
    meta:       Dict[str, Any]
...
def build_degradation_example(
    ...
    **sampler_kwargs,
    ...
    Args:
        ...
        res:                  Resolution for blob generation.
    ...
    Returns:
        DegradationExample with mask: Optional[Image.Image] (None for global_negative).
    """
    ...
    if variant == 'global_negative':
        ...
        return DegradationExample(
            image=res_aug.image, mask=None, supervised=True,
    ...
    if variant == 'clean_plus_small':
        ...
        return DegradationExample(
            image=out, mask=target_mask, supervised=True,
