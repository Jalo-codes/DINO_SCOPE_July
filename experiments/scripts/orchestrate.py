...
Usage::
    ...
    python -m experiments.scripts.orchestrate \
...
Queue JSON::

    {
      ...
      "runs": [
        {"name": "exp01", "module": "experiments.scripts.train",
         "args": {"num_epochs": 8}},
        ...
      ]
    }
...
absolute ``--run_root``.
- Entries running a training module (`'train' in module`) without an explicit
  `checkpoint_root` get `--checkpoint_root <run_root>/<name>` injected.
- Entries running an evaluation module (`'eval' in module`) without an explicit
  `summary_out` get `--summary_out <run_root>/<name>/eval_summary.json` injected.
...
``--stop_on_fail``).  A ``sweep_summary.csv`` (status + last metrics.csv row or
eval_summary.json dict per run) is written at the end and on interrupt.

NOTE: this script intentionally uses plain ``print()`` rather than
``lab_utils.logging.text.log_line`` (to keep the orchestrator importable everywhere).
"""
...
TRAINER_MODULE = 'experiments.scripts.train'
...
def _substitute(value, run_root: str):
    if isinstance(value, str):
        val = value.replace('{run_root}', run_root)
        import re
        import os
        matches = re.findall(r'\{env:([^},]+)(?:,([^}]+))?\}', val)
        for var, default in matches:
            env_val = os.environ.get(var, default or '')
            target = f'{{env:{var}{"," + default if default else ""}}}'
            val = val.replace(target, env_val)
        return val
...
def build_argv(entry: dict, base_args: dict, run_root: str) -> list[str]:
    ...
    # Only inject checkpoint_root for training modules
    if 'train' in module and 'checkpoint_root' not in merged:
        merged['checkpoint_root'] = os.path.join(run_root, name)

    # Only inject summary_out for evaluation modules
    if 'eval' in module and 'summary_out' not in merged:
        merged['summary_out'] = os.path.join(run_root, name, 'eval_summary.json')
...
def last_metrics_row(run_root: str, name: str) -> dict:
    """Read metrics for a run (either flat eval_summary.json or last metrics.csv row)."""
    entry_dir = os.path.join(run_root, name)

    # 1. Try eval_summary.json first
    eval_json = os.path.join(entry_dir, 'eval_summary.json')
