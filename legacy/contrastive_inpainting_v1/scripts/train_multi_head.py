...
from lab_utils.eval.localization import (
    collect_attention_zoom_samples,
    report_zoom_eval,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--val_zoom_thresh', type=str, choices=['gap', 'otsu', 'hyst'],
                   default='otsu',
                   help='Attention threshold for the val-zoom bbox (otsu = generous '
                        'hot/cold split; the box is drawn from attention, not GT).')
    p.add_argument('--val_zoom_pad_frac', type=float, default=0.25,
                   help='Area-inverse padding fraction for the val-zoom bbox '
                        '(smaller splices get more context, larger ones less).')
        ...
        if ctx.is_main:
            ...
            if args.val_zoom and args.contrastive_dim > 0:
                ...
                for _, zoom_items, tag in eval_sources:
                    ...
                    zsamples = collect_attention_zoom_samples(
                        eval_model, zoom_items, device, res=cfg.resolution,
                        normalize_mean=cfg.IMAGENET_MEAN, normalize_std=cfg.IMAGENET_STD,
                        decode_spec=_eval_decode_spec(args, cfg),
                        pad_frac=float(args.val_zoom_pad_frac),
                        zoom_thresh_mode=str(args.val_zoom_thresh),
                        log_tag='[eval]', tag=tag,
                    )
