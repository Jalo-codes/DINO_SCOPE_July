...
def main():
    ...
    def _embed(pil):
        """One forward of a PIL RGB image → (z (N,D) L2-normed, attention (N,))."""
        inp = normalize(TF.to_tensor(
            TF.resize(pil, [T, T], Image.BILINEAR))).unsqueeze(0).to(device)
        ...
        if z is None:
            return None, None
        zz = z[0].detach().cpu().float().numpy()
        zz = zz / (np.linalg.norm(zz, axis=1, keepdims=True) + 1e-12)
        a = out.get('pool_attention')
        aa = (a[0].detach().cpu().float().numpy() if a is not None
              else np.zeros(zz.shape[0], dtype=np.float32))
        return zz.astype(np.float32), aa.astype(np.float32)
    ...
    for split, items in by_split.items():
        for it in items:
            ...
            z_np, att_np = _embed(source)
            if z_np is None:
                continue

            z_all.append(z_np)
            att_all.append(att_np)
