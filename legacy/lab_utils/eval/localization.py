...
from lab_utils.data.resolution import (
    Resolution, resize_only, resize_only_mask,
)
...
@torch.no_grad()
def _decode_labels(z_np, decode_spec, decode_fn=None, attention=None):
    """(N,) {0,1} cluster labels. ``decode_fn(z, attention)`` (e.g. an HDBSCAN
    adapter that lives OUTSIDE lab_utils) overrides ``decode_spec`` when given —
    so the hdbscan dependency never enters the eval utility."""
    if decode_fn is not None:
        return np.asarray(decode_fn(z_np, attention)).reshape(-1)
    return decode_oracle_labels(z_np, decode_spec)


def _decode_oracle_mask(
    z_np: Optional[np.ndarray], gt_px: Optional[np.ndarray], res: Resolution,
    decode_spec: DecodeSpec = DecodeSpec(), *, decode_fn=None, attention=None,
) -> Tuple[np.ndarray, float, float]:
    """Decode → attention-guided pixel MASK + (f1, iou) vs gt_px (frame-sized).

    Returns the chosen-polarity (S, S) bool mask alongside its metrics. Zeros + (0, 0)
    when the embedding is missing or the GT is empty.
    """
    S = res.image_size
    if z_np is None or gt_px is None or not gt_px.any():
        return np.zeros((S, S), dtype=bool), 0.0, 0.0
    raw = _decode_labels(z_np, decode_spec, decode_fn, attention)
    P, ps = res.num_patches_per_side, res.patch_size
    if decode_spec.method in ('kmeans',) and decode_fn is None:
        pred_patches = _select_cluster(raw, attention)
    else:
        pred_patches = (raw == 1)
    px = _patches_to_pixels(pred_patches, P, ps)
    f1, iou = _f1_iou(px, gt_px)
    return px, f1, iou
...
def _oracle_pixel_from_z(
    ...
    """Decode → attention-guided PIXEL (f1, iou) vs gt_px (bool, frame-sized)."""
    _, f1, iou = _decode_oracle_mask(z_np, gt_px, res, decode_spec)
    return f1, iou


def _place_crop_in_frame(crop_mask_ss, bbox_native, src_wh, res):
    """NN-place an (S,S) crop-frame bool mask back into the (S,S) display frame at
    the native-pixel bbox's location; zeros outside the box."""
    S = res.image_size
    W, H = src_wh
    x0, y0, x1, y1 = bbox_native
    sx0 = max(0, min(S - 1, int(round(x0 * S / W))))
    sx1 = max(sx0 + 1, min(S, int(round(x1 * S / W))))
    sy0 = max(0, min(S - 1, int(round(y0 * S / H))))
    sy1 = max(sy0 + 1, min(S, int(round(y1 * S / H))))
    pil = Image.fromarray((crop_mask_ss.astype(np.uint8) * 255), mode='L').resize(
        (sx1 - sx0, sy1 - sy0), Image.NEAREST)
    placed = np.zeros((S, S), dtype=bool)
    placed[sy0:sy1, sx0:sx1] = np.asarray(pil) > 127
    return placed


def _decode_zoom_placed(
    z_z, bbox_native, src_wh, gt_full, res, decode_spec=DecodeSpec(),
    *, decode_fn=None, attention=None, full_pred=None,
) -> Tuple[np.ndarray, np.ndarray, float, float]:
    """Decode the zoom crop, place the attention-guided polarity mask back
    into the full (S,S) frame, and score vs the FULL GT. Returns
    (zoom_pred_full (S,S), crop_pred (S,S crop frame), f1, iou).
    """
    S = res.image_size
    if z_z is None or gt_full is None or not gt_full.any():
        z = np.zeros((S, S), dtype=bool)
        return z, z.copy(), 0.0, 0.0
    raw = _decode_labels(z_z, decode_spec, decode_fn, attention)
    P, ps = res.num_patches_per_side, res.patch_size
    if decode_spec.method in ('kmeans',) and decode_fn is None:
        pred_patches = _select_cluster(raw, attention)
    else:
        pred_patches = (raw == 1)
    crop_pred = _patches_to_pixels(pred_patches, P, ps)
    zoom_pred_full = _place_crop_in_frame(crop_pred, bbox_native, src_wh, res)

    # Fallback: if the zoom prediction is empty, defer to the top-level full_pred restricted to the bbox
    if zoom_pred_full.sum() == 0 and full_pred is not None and full_pred.any():
        bbox_mask = np.zeros((S, S), dtype=bool)
        x0, y0, x1, y1 = bbox_native
        sx0 = max(0, min(S - 1, int(round(x0 * S / src_wh[0]))))
        sx1 = max(sx0 + 1, min(S, int(round(x1 * S / src_wh[0]))))
        sy0 = max(0, min(S - 1, int(round(y0 * S / src_wh[1]))))
        sy1 = max(sy0 + 1, min(S, int(round(y1 * S / src_wh[1]))))
        bbox_mask[sy0:sy1, sx0:sx1] = True

        zoom_pred_full = full_pred & bbox_mask

        # Reconstruct crop_pred for visualization panels
        crop_sub = full_pred[sy0:sy1, sx0:sx1]
        if crop_sub.any():
            pil = Image.fromarray((crop_sub.astype(np.uint8) * 255), mode='L').resize((S, S), Image.NEAREST)
            crop_pred = np.asarray(pil) > 127
        else:
            crop_pred = np.zeros((S, S), dtype=bool)

    f1, iou = _f1_iou(zoom_pred_full, gt_full)
    return zoom_pred_full, crop_pred, f1, iou


@dataclass
class AttnZoomViz:
    """Per-item artifacts from ``attention_zoom_item`` for visualizing the
    attention-zoom eval. ``sample`` carries the same metrics the collector
    aggregates; the rest are the drawables (all GT-free except for scoring)."""
    sample: '_ZoomSample'
    disp: np.ndarray                              # (S, S, 3) uint8 full frame
    gt_full: np.ndarray                           # (S, S) bool
    attn_grid: Optional[np.ndarray]               # (P, P) float full-frame attention
    bbox: Optional[Tuple[int, int, int, int]]     # native-px (x0, y0, x1, y1) zoom box
    full_pred: np.ndarray                         # (S, S) bool full-frame decode
    zoom_pred_full: np.ndarray                    # (S, S) bool zoom decode PLACED BACK in full frame
    crop_disp: Optional[np.ndarray]               # (S, S, 3) uint8 resized zoom crop
    crop_attn: Optional[np.ndarray]               # (P, P) float zoom-crop attention
    crop_pred: Optional[np.ndarray]               # (S, S) bool zoom-crop decode (crop frame)


@torch.no_grad()
def attention_zoom_item(
    model: nn.Module,
    src: Image.Image,
    mask: Image.Image,
    gt_full: np.ndarray,
    *,
    ...
    zoom_thresh_mode: str = 'otsu',
    zoom_thresh_mult: float = 0.85,
    decode_fn=None,
    kind: str = '',
) -> AttnZoomViz:
    """One splice → FULL vs ATTENTION-zoom result + drawables.

    The zoom crop's bbox is drawn straight from the pooled attention: threshold it
    (default ``otsu`` — the generous hot/cold split), tight-box every attended
    patch, pad inversely to the detected area via ``attention_zoom_bbox`` /
    ``get_padded_bbox``. The GT mask is used ONLY to score (oracle polarity),
    never to place the crop.

    The ZOOM iou is the crop's decode PLACED BACK into the full frame and scored
    vs the FULL GT (so a box that clips the splice is penalized, and it's directly
    comparable to FULL iou). It is the zoom mask ALONE — never AND/OR'd with the
    full-frame mask. ``decode_fn(z, attention)`` overrides ``decode_spec`` (used to
    drive HDBSCAN from graph_lab without importing it into lab_utils).
    """
    ...
    # FULL frame: decode + oracle-polarity mask/IoU vs the full GT.
    z_f, logit_f, attn_f = _embed_logit_attn_pil(
        model, src, res, device, normalize_mean, normalize_std)
    full_pred, f1_f, iou_f = _decode_oracle_mask(
        z_f, gt_full, res, decode_spec, decode_fn=decode_fn, attention=attn_f)
    attn_grid = attn_f.reshape(P, P) if attn_f is not None else None

    # ATTENTION-driven crop (GT-free): bbox straight from the attention map.
    bbox = None
    if attn_f is not None:
        bbox = attention_zoom_bbox(
            attn_grid, H, W, base_padding=base_padding, pad_frac=pad_frac,
            min_crop_patches=min_crop_patches, thresh_mode=zoom_thresh_mode,
            thresh_mult=zoom_thresh_mult)

    crop_disp = crop_attn = crop_pred = None
    if bbox is None:
        # Flat attention → no hot region to zoom into; ZOOM := FULL.
        zoom_pred_full = full_pred
        f1_z, iou_z, realized, logit_z = f1_f, iou_f, area_full, logit_f
    else:
        x0, y0, x1, y1 = bbox
        crop_img  = src.crop((x0, y0, x1, y1))
        crop_mask = mask.crop((x0, y0, x1, y1))
        m_native = np.asarray(crop_mask, dtype=np.uint8) > 0
        realized = float(m_native.mean()) if m_native.size else 0.0
        z_z, logit_z, attn_z = _embed_logit_attn_pil(
            model, crop_img, res, device, normalize_mean, normalize_std)
        zoom_pred_full, crop_pred, f1_z, iou_z = _decode_zoom_placed(
            z_z, bbox, src.size, gt_full, res, decode_spec,
            decode_fn=decode_fn, attention=attn_z, full_pred=full_pred)
        crop_disp = np.asarray(resize_only(crop_img, res), dtype=np.uint8)
        crop_attn = attn_z.reshape(P, P) if attn_z is not None else None

    sample = _ZoomSample(
        kind=kind, area_full=area_full,
        target_cov=area_full, realized_cov=realized, bucket_zoom=_bucket(realized),
        f1_full=f1_f, iou_full=iou_f, f1_zoom=f1_z, iou_zoom=iou_z,
        f1_oracle=f1_z, iou_oracle=iou_z,   # oracle removed; mirror zoom for report compat
        bce_logit_full=logit_f, bce_logit_zoom=logit_z,
    )
    return AttnZoomViz(
        sample=sample, disp=disp, gt_full=gt_full, attn_grid=attn_grid, bbox=bbox,
        full_pred=full_pred, zoom_pred_full=zoom_pred_full, crop_disp=crop_disp,
        crop_attn=crop_attn, crop_pred=crop_pred)


@torch.no_grad()
def collect_attention_zoom_samples(
    model: nn.Module,
    items: List[Dict],
    device: torch.device,
    *,
    ...
    """FULL vs ATTENTION-zoom localization per splice — the GT-free replacement
    for the old coverage probe. Thin loop over ``attention_zoom_item`` (the shared
    per-item core, so the analysis viz uses the exact same path). Reals skipped.
    ``decode_fn(z, attention)`` overrides ``decode_spec`` (HDBSCAN from graph_lab).
    """
    model.eval()
    suffix = f' {tag}' if tag else ''
    ...
    for it in items:
        ...
        viz = attention_zoom_item(
            ...
            decode_spec=decode_spec, base_padding=base_padding, pad_frac=pad_frac,
            min_crop_patches=min_crop_patches, zoom_thresh_mode=zoom_thresh_mode,
            zoom_thresh_mult=zoom_thresh_mult,
            decode_fn=decode_fn, kind=it.get('kind', ''))
        samples.append(viz.sample)
        n_zoomed += int(viz.bbox is not None)

    log_line(f'{log_tag}{suffix} attn-zoom collected n_splice={len(samples)} '
             f'zoomed={n_zoomed} thresh={zoom_thresh_mode} mult={zoom_thresh_mult:.2f} '
             f'pad_frac={pad_frac} min_crop={min_crop_patches}')
    return samples
...
def report_zoom_eval(
    ...
    """Report zoom-eval views without size-bucketing, only aggregate."""
    suffix = f' {tag}' if tag else ''
    if not samples:
    ...
    # ── Condensed mode: aggregate FULL + ZOOM only ──
    if condensed:
        full_iou = _stats([s.iou_full for s in samples])
        zoom_iou = _stats([s.iou_zoom for s in samples])
        full_f1 = _stats([s.f1_full for s in samples])
        zoom_f1 = _stats([s.f1_zoom for s in samples])
        ...
        out['aggregate'] = {'full': full_iou, 'zoom': zoom_iou,
                            'full_f1': full_f1, 'zoom_f1': zoom_f1}
        return out

    # ── Full mode: aggregate FULL / ZOOM / ORACLE + confidence + delta ──
    full_iou = _stats([s.iou_full for s in samples])
    zoom_iou = _stats([s.iou_zoom for s in samples])
    orac_iou = _stats([s.iou_oracle for s in samples])
    full_f1 = _stats([s.f1_full for s in samples])
    zoom_f1 = _stats([s.f1_zoom for s in samples])
    orac_f1 = _stats([s.f1_oracle for s in samples])
    cov_med = float(np.median([s.realized_cov for s in samples])) if samples else 0.0

    log_line(f'{log_tag}{suffix} zoom aggregate cov_med={cov_med:.2f} '
             f'FULL   iou {_pct_line(full_iou)} f1 {_pct_line(full_f1)}')
    log_line(f'{log_tag}{suffix} zoom aggregate cov_med={cov_med:.2f} '
             f'ZOOM   iou {_pct_line(zoom_iou)} f1 {_pct_line(zoom_f1)}')
    log_line(f'{log_tag}{suffix} zoom aggregate cov_med={cov_med:.2f} '
    ...
    out['aggregate'] = {
        'full': full_iou, 'zoom': zoom_iou, 'oracle': orac_iou,
        'realized_cov_med': cov_med, 'full_f1': full_f1,
        'zoom_f1': zoom_f1, 'oracle_f1': orac_f1
    }

    # ── B. detector-confidence × zoom (view-matched zoom logit) ──
    if all(s.bce_logit_zoom is not None for s in samples):
        thr = 0.0 if t_op is None else float(t_op)
        det = [s.bce_logit_zoom for s in samples if s.bce_logit_zoom >= thr]
        conf_cut = float(np.median(det)) if det else thr
        log_line(f'{log_tag}{suffix} zoom CONFIDENCE (zoom-logit: '
                 f'missed<{thr:+.2f} <=less< {conf_cut:+.2f} <=confident) '
                 f'— detection/localization entanglement')
        ...
        for k in ('confident', 'less', 'missed'):
            ...
            row[k] = st
            if st['n']:
                log_line(f'{log_tag}{suffix} zoom aggregate det={k:<9} '
                         f"n={st['n']:<3} iou_med={st['median']:.3f} "
                         f"iou_p25={st['p25']:.3f} iou_mean={st['mean']:.3f}")
        out['confidence'] = row
