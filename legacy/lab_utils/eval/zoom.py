...
def compute_hysteresis_mask(
    ...
    *,
    ...
    flat = att.reshape(-1)
    strong = compute_otsu_threshold(flat) * thresh_mult
    seeds = att >= strong
    if not seeds.any():
        return seeds
...
def attention_zoom_bbox(
    ...
    *,
    ...
    Args:
        ...
        thresh_mult:      multiplier applied to computed thresholds (default 1.0)
    ...
    if thresh_mode == 'hyst':
        hot = compute_hysteresis_mask(att_nn, thresh_mult=thresh_mult)
    elif thresh_mode == 'otsu':
        hot = att_nn >= (compute_otsu_threshold(att_flat) * thresh_mult)
    else:
        hot = att_nn >= (compute_gap_threshold(att_flat) * thresh_mult)
...
def multi_zoom_bboxes(
    ...
    *,
    ...
    if hot_mask is not None:
        ...
        att_flat = att_nn.reshape(-1)
        if thresh_mode == 'hyst':
            H = compute_hysteresis_mask(att_nn, thresh_mult=thresh_mult)
        elif thresh_mode == 'otsu':
            H = att_nn >= (compute_otsu_threshold(att_flat) * thresh_mult)
        else:
            H = att_nn >= (compute_gap_threshold(att_flat) * thresh_mult)
